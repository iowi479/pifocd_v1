...
void ShapingQueue::push(PIFOPacket packet) {
  ...
  if (!isLeaf)
    throw cRuntimeError("ShapingQueue(id=%d) is not a leaf", id);
  ...
  EV_INFO << "ShapingQueue(id=" << (int)this->id
...
PIFOPacket ShapingQueue::pull() {
  if (sq.size() < pq.size()) {
    ...
    if (!isLeaf)
      throw cRuntimeError("ShapingQueue(id=%d) is not a leaf", id);
    if (!std::holds_alternative<Packet *>(pq_entry.value))
      throw cRuntimeError(
          "ShapingQueue(id=%d) pq_entry doesn't hold a Packet *", id);
    ...
    EV_INFO << "pulling from queue in weird state: sq.size() = " << sq.size()
...
PIFOPacket ShapingQueue::peek() const {
  ...
  if (sq.size() < pq.size()) {
    ...
    PFIOPacket packet = std::get<PIFOPacket>(pq_entry.value);
    ...
    // something weird. Fall back
    EV_INFO << "peeking at queue in weird state: sq.size() = " << sq.size()
            << ", pq.size() = " << pq.size() << EV_ENDL;
...
void ShapingQueue::wake() {
  ...
  EV_INFO << "ShapingQueue(id=" << (int)this->id << ") got woken @" << now
          << EV_ENDL;
